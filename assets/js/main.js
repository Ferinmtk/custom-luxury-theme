...
(function () {
	...
	var seekVideo = function (vid, t) {
		if (vid.readyState < 1) { return; }           // no metadata, no duration
		if (vid.seeking) { return; }                  // a seek is already pending
		if (Math.abs(vid.currentTime - t) <= 0.01) { return; }
		...
		if (!isBuffered(vid, t)) {
			...
			var now = Date.now();
			if (now - (vid._lhLastFetchSeek || 0) < 250) { return; }
			vid._lhLastFetchSeek = now;
		}

		try { vid.currentTime = t; } catch (e) { /* not seekable yet */ }
	};
	...
	if (seq && vid) {
		...
		if (reducedMotion) {
			...
			var onScroll = function () {
				...
				if (alt) { alt.classList.toggle('on-film', grow > 0.55); }
	...
	(function () {
		...
		if (reducedMotion || isTouch) { scrub.classList.add('is-static'); return; }
		...
		function measure() { maxX = Math.max(0, track.scrollWidth - stage.clientWidth); }
		...
		function readScroll() {
			...
			trackX = -moveP * maxX;
			if (prog) { prog.style.width = (moveP * 100) + '%'; }
			if (progBar) { progBar.classList.toggle('is-complete', moveP > 0.995); }
		}
	...
	(function () {
		...
		function update() {
			...
			for (var i = 0; i < els.length; i++) {
				...
				if (r.bottom < 0 || r.top > h) { continue; } // off-screen -> skip
		...
		window.addEventListener('scroll', function () {
			if (!pp) {
				pp = true;
				requestAnimationFrame(function () { update(); pp = false; });
			}
		}, { passive: true });
	...
	if (scrubReveals.length && 'IntersectionObserver' in window && !reducedMotion) {
		...
		scrubReveals.forEach(function (el) { sio.observe(el); });
	} else {
		scrubReveals.forEach(function (el) { el.classList.add('in'); });
	}
		...
		function render(force) {
			...
			cards.forEach(function (card) {
				...
				if (reducedMotion) {
					...
					setTimeout(function () {
						if (card.classList.contains('is-faded')) { card.classList.add('is-hidden'); }
					}, 320);
				}
			});
			applyRhythm(shown);
		...
		if (bar) {
			...
			if (canSpring) {
				...
				function tossTrack(tilt) {
					return [
						{ offset: 0.00, translate: '0 100px',  rotate: tilt + 'deg', easing: 'cubic-bezier(0.17, 0.62, 0.34, 1)' },   // rise from 100px below, decelerating
						{ offset: 0.30, translate: '0 -100px', rotate: tilt + 'deg', easing: 'linear' },                              // apex — HANG (no motion)
						{ offset: 0.60, translate: '0 -100px', rotate: tilt + 'deg', easing: 'cubic-bezier(0.55, 0, 0.85, 0.5)' },    // gravity takes over, accelerating
			...
			bar.addEventListener('click', function (e) {
				...
				if (!btn) { return; }
				bar.querySelectorAll('.hm-filter').forEach(function (b) { b.classList.remove('is-active'); b.setAttribute('aria-pressed', 'false'); });
	...
	(function () {
		...
		var copies = ['b1', 'b2', 'b3', 'b4'].map(function (id) { return document.getElementById(id); });
		...
		vid.addEventListener('loadedmetadata', function () { dur = vid.duration; vid.pause(); });
	...
	(function () {
		...
		if (reducedMotion) { return; }   // CSS already shows the finished drawing
			...
			window.setTimeout(function () {
				['hww-m1', 'hww-m2', 'hww-m3'].forEach(function (id) {
					...
					if (el && el.beginElement) {
						try { el.beginElement(); } catch (e) { /* SMIL unsupported */ }
					}
				});
	...
	(function () {
		...
		overlay.addEventListener('click', function (e) {
			if (e.target.closest('a[href]')) { setOpen(false); return; }
	...
	(function () {
		...
		function setErr(step, msg) { var e = step.querySelector('.si-err'); if (e) { e.textContent = msg || ''; } }
		function hasText(step) {
			var ins = step.querySelectorAll('.si-inp');
			for (var k = 0; k < ins.length; k++) { if (ins[k].value.trim() !== '') { return true; } }
			return false;
		}
		function cue(t) { if (note.lastChild) { note.lastChild.textContent = t; } }
		...
		function validate(i) {
			...
			for (var k = 0; k < reqs.length; k++) {
				...
				if (el.type === 'radio') {
					...
					if (!picked) { setErr(step, 'Please choose an option.'); return false; }
		...
		function refreshNav() {
			...
			if (auto) {
				next.hidden = true; send.hidden = true;
				note.style.display = 'flex'; cue('Select an answer to continue');
			} else if (last) {
				next.hidden = true; send.hidden = false; note.style.display = 'none';
			} else if (optional) {
				next.hidden = false; send.hidden = true; note.style.display = 'none';
			} else {
				var t = hasText(step);
				next.hidden = !t; send.hidden = true;
				note.style.display = t ? 'none' : 'flex'; cue('Type your answer to continue');
			}
		}
		...
		form.addEventListener('change', function (e) {
			...
			group.forEach(function (r) { r.closest('.si-opt').classList.toggle('is-sel', r.checked); });
			var step = e.target.closest('.si-step');
			if (step && step.getAttribute('data-auto') === '1') {
				setTimeout(function () { advance(); }, 340);
			}
		});

		/* Typed steps: reveal Continue once they start typing */
		form.addEventListener('input', function (e) {
			if (!e.target.classList.contains('si-inp')) { return; }
			var step = steps[current];
			if (step.getAttribute('data-auto') === '1' || step.getAttribute('data-optional') === '1' || current === total - 1) { return; }
			var t = hasText(step);
			next.hidden = !t;
		...
		back.addEventListener('click', function () { show(Math.max(0, current - 1), -1); });
	...
	(function () {
		...
		if (window.matchMedia('(hover: hover)').matches) { return; } // desktop uses CSS hover
		cards.forEach(function (card) {
