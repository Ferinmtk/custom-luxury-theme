    ...
    <main id="main" class="si-page">
        <?php while ( have_posts() ) : the_post(); ?>
            <section class="si-wrap">
                <?php
                /* The head used to sit inside the card. Two headlines stacked would have
                   fought each other, so it moves out wholesale rather than a second one
                   being written — which is also what shrinks the card: what is left of
                   it is exactly the form. */
                ?>
                <header class="si-hero">
                    <div class="si-hero-in">
                ...
                <?php
                /* Band, after the lede. Full layout width, nothing overlapping it.
                   The band is 520px tall rather than the 330 it started at: the couple
                   occupies more than half the frame's height, and anything shorter cut
                   them at the waist. At 1240 wide cover scales the picture down, not up,
                   so it is sharp at 1x — retina will show some softness and a larger
                   original would fix it. */
                $ct_band = lh_field_image( 'contact_band_image', 'img/contact-band.jpg', 'full', array(
                        'alt'      => '',
                        'loading'  => 'eager',
                        'decoding' => 'async',
                        'width'    => '1344',
                ...
                if ( $ct_band ) : ?>
                    <div class="si-hero-band" aria-hidden="true"><?php echo $ct_band; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- built by lh_field_image ?></div>
                <?php endif; ?>

                <?php
                /* The form and details sit on a ground: a second photograph at low
                   opacity under a diagonal wash. Scoped to .si-formzone rather than the
                   whole section, so it stops before the map — the map is already a large
                   image and does not want another behind it.

                   contact-ground.jpg is the studio interior cropped at 57% width; the
                   lit panel beyond that reads CUSTOM LUXURY BUILDERS, which is not this
                   company. A different photograph from the band on purpose: the same one
                   twice, 250px apart, would read as a mistake. */
                $ct_ground = lh_field_image( 'contact_ground_image', 'img/contact-ground.jpg', 'large', array(
                        'alt'      => '',
                        'loading'  => 'lazy',
                        'decoding' => 'async',
                        'width'    => '1069',
                ...
                ?>
                <div class="si-formzone">
                    <?php if ( $ct_ground ) : ?>
                        <div class="si-bg" aria-hidden="true">
                            <?php echo $ct_ground; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- built by lh_field_image ?>
                            <span class="si-bg-wash"></span>
                        </div>
                    <?php endif; ?>
                    <div class="si-layout">
                        <div class="si-stage">
                                            ...
                                            <div class="si-steps">
                                                ...
                                                <fieldset class="si-step" data-field="Coordinates">
                                                    ...
                                                    <div class="si-inp-wrap">
                                                        ...
                                                        <div class="si-fieldline"><input class="si-inp" id="lh_zip" name="lh_zip" inputmode="numeric" maxlength="10" data-req="1" autocomplete="postal-code" placeholder="80113"></div>
                                                        <p class="si-hint">So we can confirm your area along the Front Range.</p>
                                                    </div>
                                                    <p class="si-err" aria-live="polite"></p>
                                                </fieldset>
                                                ...
                                                <fieldset class="si-step" data-field="Phone">
                                                    ...
                                                    <div class="si-inp-wrap">
                                                        ...
                                                        <div class="si-fieldline"><input class="si-inp" id="lh_phone" name="lh_phone" type="tel" data-req="1" autocomplete="tel" placeholder="(303) 555-0142"></div>
                        ...
                        <aside class="si-side">
                            <div class="si-detail">
                                <h3>Phone</h3>
                                <p><a href="tel:<?php echo esc_attr( preg_replace( '/[^0-9+]/', '', lh_field( 'phone', '+13035550100' ) ) ); ?>"><?php echo esc_html( lh_field( 'phone_display', '(303) 555-0100' ) ); ?></a></p>
                            </div>
                            <div class="si-detail">
                                <h3>Email</h3>
                ...
                <div class="si-maparea">
                    <div class="si-maphead">
                        ...
                        <a class="si-maplink" href="https://www.google.com/maps/search/?api=1&amp;query=Denver,+Colorado" target="_blank" rel="noopener">Open in Google Maps &rarr;</a>
                    </div>
